...
switch($menu->get(2)){ 
    case "new":			
        if( db_query("INSERT INTO prefix_raid_dkps (inzen, name, dkp) 
        VALUES('".
        escape($menu->get(1), 'integer')."','".
        escape($_POST['name'], 'string')."','".
        escape($_POST['dkp'], 'integer')."'; ") ){
    ...
    case "edit":
        ...
        if( db_query("
                ...
                SET 
                    ...
                    dkp='".escape($_POST['dkp'], 'integer')."' 
                WHERE id=".escape($_POST['id'],'integer') 
            )
        ){
    ...
    case "del":
        if( db_query("DELETE FROM prefix_raid_dkps WHERE id = '".$menu->get(3)."' LIMIT 1") ){
            wd('admin.php?raiddkps-'.$menu->get(1),'L&ouml;schen war erfolgreich!');
        }else{
            wd('admin.php?raiddkps-'.$menu->get(1),'L&ouml;schen war <b>nicht</b> erfolgreich!');
        }
    break;
    default:
        $tpl->out(0); #Header
        ...
        if( $menu->get(1) == 0 ){
            ...
            $inzen = db_result(db_query("SELECT name FROM prefix_raid_inzen WHERE id=".$menu->get(1)." LIMIT 1" ), 0);
            $dungeon_id = $menu->get(1);
        }
        $tpl->set_out("inzen", $inzen, 9);
        if( $menu->get(1) != '' ){
            $res = db_query("SELECT id, name, dkp FROM prefix_raid_dkps WHERE inzen=".$dungeon_id );
            if( db_num_rows( $res ) ){
                while( $boss = db_fetch_object($res) ){
                    $boss->edit = "<a href='admin.php?raiddkps-".$dungeon_id."-".$boss->id."'><img src='include/images/icons/edit.gif'></a>";
                    $boss->del = "<a href='admin.php?raiddkps-".$dungeon_id."-del-".$boss->id."'><img src='include/images/icons/del.gif'></a>";
                    $tpl->set_ar_out( $boss, 10);
                }
            }else{
                $tpl->set_out("msg","Kein DKP f&ouml;r \"<b>".$inzen."</b>\" Gefunden!", 11);
            }
        }else{
                $tpl->set_out("msg","Bitte Instanze Ausw&auml;hlen in der Mitte!", 11);
        }
        $tpl->out(12);
        ### Boss Liste ENDE
