...
switch($menu->get(1)){
    ...
    case "new":
        getPost('id');
        if(  $raid->db()->singel()->insert('raid_inzen')->fields($_POST)->init() ){
            wd('admin.php?raidinzen','Neuer eintrag war erfolgreich!', 3);
        }else{
    ...
    case "edit":
        if( $stat and $raid->db()->singel()->update('raid_inzen')->where(array('id' => getPost('id')))->fields($_POST)->init() ){
    ...
    break;
    case "del":
        if( $menu->get(3) == "TRUE" || $raid->permission()->delete('fast') ){
            if( $raid->db()->singel()->delete('raid_inzen')->where(array('id' => $menu->get(2)))->init() ){
